...
Para implantar em servidores com recursos limitados (como Oracle Cloud Always Free), utilize o Docker.
...
```bash
docker build -t carometro .
```

### 2. Rodar o Container

Execute o container mapeando um volume para persistir as fotos:
...
docker run -d \
  ...
  -p 3000:3000 \
  --restart=always \
  -v ~/fotos-alunos:/app/public/photos \
  --memory="512m" \
  carometro
```

**Nota**: O parâmetro `-v ~/fotos-alunos:/app/public/photos` garante que as fotos tiradas não sejam perdidas se o container for reiniciado. As fotos ficarão salvas na pasta `~/fotos-alunos` do seu servidor (host).
...
- A foto será salva no diretório `public/photos` (ou no volume mapeado), e o cartão do aluno será atualizado para mostrar a nova imagem.
